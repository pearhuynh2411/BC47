...
select COUNT(u.user_id) AS 'số like' ,u.full_name from users u
...
select COUNT(r.res_id),r.res_id, r.res_name from restaurant r
...
select COUNT(u.user_id) AS 'số order' ,u.user_id, u.full_name from users u
inner join orders o on u.user_id = o.user_id
group by u.user_id, u.full_name
ORDER BY COUNT(u.user_id) DESC

#Tìm người dùng không hoạt động trong hệ thống (không đặt hàng, không like, không đánh giá nhà hàng).
SELECT COUNT(u.user_id) AS 'users not work', u.user_id, u.full_name from users u
